...
export const getDevices = (
  params: Partial<DeviceFilters>
): Promise<AxiosResponse<Device[]>> => API.get("/devices", { params });

export const getDevice = (
  id: number | string
): Promise<AxiosResponse<Device>> => API.get(`/devices/${id}`);

export const addDevice = (
  data: Partial<DeviceForm>
): Promise<AxiosResponse<{ message: string; id: number; uniqueId: string }>> =>
...
export const deleteDevice = (
  id: number
): Promise<AxiosResponse<{ message: string }>> => API.delete(`/devices/${id}`);
...
export const getDeletionRequests = (): Promise<
  AxiosResponse<DeletionRequest[]>
...
export const addNews = (data: {
  ...
  content: string;
  category: string;
}): Promise<AxiosResponse<{ message: string }>> => API.post("/news", data);
