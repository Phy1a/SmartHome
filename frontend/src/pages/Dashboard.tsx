...
function SimpleDashboard({ user, stats, alerts }: SimpleDashboardProps) {
  ...
  const progress = lvl.needed
    ? Math.min(((user?.points ?? 0) / lvl.needed) * 100, 100)
    : 100;
      ...
      <div className="xp-bar" style={{ marginBottom: 24 }}>
        <div className="xp-header">
          <div>
            ...
            >
              Bienvenue, {user?.firstName || user?.username} {lvl.icon}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span className="xp-points">{user?.points?.toFixed(2)} pts</span>
              <span style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
                Niveau:{" "}
                <strong style={{ color: "white" }}>{user?.level}</strong>
              </span>
            </div>
          </div>
          <div
          ...
          >
            {lvl.next ? (
              <>
                Prochain:{" "}
                <strong style={{ color: lvl.color }}>{lvl.next}</strong> (
                {lvl.needed} pts)
              </>
            ) : (
              "🏆 Niveau maximum"
          ...
          {(
            ...
            <span
              ...
              style={{
                ...
                border: "1px solid",
                borderColor:
                  user?.level === l ? lvl.color : "rgba(255,255,255,0.15)",
        ...
        <div className="stat-grid" style={{ marginBottom: 24 }}>
          <div className="stat-card">
            ...
            <div>
              ...
              <div className="stat-label">Appareils</div>
            </div>
          </div>
          <div className="stat-card">
...
function AdvancedDashboard({
  ...
  useEffect(() => {
    getDevices({})
      .then((r) => setDevices(r.data.slice(0, 8)))
      .catch(() => {});
  }, []);
  ...
  const handleToggle = async (device: Device) => {
    ...
    try {
      ...
      setDevices((prev) =>
        prev.map((d) =>
          d.id === device.id
            ? { ...d, status: res.data.status as "actif" | "inactif" }
            : d,
        ),
      );
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            Tableau de bord — {user?.level?.toUpperCase()} {icon}
          </div>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>
            {user?.loginCount} connexions · {user?.actionCount} actions
          </div>
        </div>
      </div>
      ...
      {stats && (
        <div className="stat-grid" style={{ marginBottom: 24 }}>
          <div className="stat-card">
            ...
            <div>
              <div className="stat-value">{stats.total}</div>
              <div className="stat-label">Appareils total</div>
            </div>
          </div>
          <div className="stat-card">
            <div className="stat-icon green">✅</div>
        ...
        >
          {devices.map((d: Device) => (
            ...
            >
              ...
              <label
                className="toggle"
                title={d.status === "actif" ? "Désactiver" : "Activer"}
              >
                <input
                  type="checkbox"
                  checked={d.status === "actif"}
